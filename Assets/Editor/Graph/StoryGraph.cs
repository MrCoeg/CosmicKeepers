...
namespace Subtegral.DialogueSystem.Editor
{
    public class StoryGraph : EditorWindow
    {
        ...
        public StoryGraphView _graphView { get; private set; }
        ...
        public void OpenGraphViewWindow(DialogueContainer container)
        {
            _fileName = container.name;
            reGenerateToolbar();
            GraphSaveUtility.GetInstance(_graphView).LoadNarrative(container.name, false);
            var window = GetWindow<StoryGraph>();
            window.titleContent = new GUIContent("Narrative Graph");
        }

        private GraphSaveUtility GetGraphUtilInstance()
        {
            return GraphSaveUtility.GetInstance(_graphView);
        }
        ...
        private void RequestDataOperation(bool save)
        {
            if (!string.IsNullOrEmpty(_fileName))
            {
                ...
                if (save)
                    ...
                    saveUtility.LoadNarrative(_fileName, false);
            }
            else
            {
        ...
        private void OnGUI()
        {
            ...
            if (Event.current.type == EventType.DragExited)
            {
                ...
                fileName = fileName.Split('.')[0];
                    ...
                    var saveUtility = GraphSaveUtility.GetInstance(_graphView);
                    saveUtility.LoadNarrative(fileName, false);
            }
        }
    }
}